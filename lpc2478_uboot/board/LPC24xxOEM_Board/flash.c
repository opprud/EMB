...
ulong flash_init (void)
{
  ...
  flash_info[1].size = 0x00100000;  /* 1 MB */
  flash_info[1].sector_count = 256;
  memset (flash_info[1].protect, 0, 256);
  ...
  flash_info[2].flash_id = (SST_MANUFACT & FLASH_VENDMASK);
  flash_info[2].size = 0x00100000;  /* 1 MB */
  flash_info[2].sector_count = 256;
  memset(flash_info[2].protect, 0, 256);
  flashbase = SST_BASEADDR + 0x00400000;
  for (j=0; j<256; j++) {
    flash_info[2].start[j] = flashbase;
    flashbase += 0x1000;  /* 4 KB sectors */
  }
  size += flash_info[2].size;

  flash_info[3].flash_id = (SST_MANUFACT & FLASH_VENDMASK);
  flash_info[3].size = 0x00100000;  /* 1 MB */
  flash_info[3].sector_count = 256;
  memset(flash_info[3].protect, 0, 256);
  flashbase = SST_BASEADDR + 0x00200000;
  for (j=0; j<256; j++) {
    flash_info[3].start[j] = flashbase;
    flashbase += 0x1000;  /* 4 KB sectors */
  }
  size += flash_info[3].size;

  flash_info[4].flash_id = (SST_MANUFACT & FLASH_VENDMASK);
  flash_info[4].size = 0x00100000;  /* 1 MB */
  flash_info[4].sector_count = 256;
  memset(flash_info[4].protect, 0, 256);
  flashbase = SST_BASEADDR + 0x00600000;
  for (j=0; j<256; j++) {
    flash_info[4].start[j] = flashbase;
    flashbase += 0x1000;  /* 4 KB sectors */
  }
  size += flash_info[4].size;

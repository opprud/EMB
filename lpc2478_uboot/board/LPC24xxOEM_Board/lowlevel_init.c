...
**
** Description:     USB Host initialization
** Parameters:      None
** Returned value:  None
**
*****************************************************************************/
...
UsbHostInit(void)
{
	...
	while (OTG_CLK_STAT != 0x00000001);

	PINSEL0 |= ((1 << 24) | (1 << 26));
	PINSEL1 |= ((1 << 30) | (1 << 26) | (1 << 28));
	PINSEL3 |= ((2 << 28) | (1 << 30));
}
...
printBanner(void)
{
  ...
  static const unsigned char message[] =
    "\n\n\r**********************************************\n\r"
          ...
          "*              EA-version: 1.9               *\n\r"
          "*     (support for 16-bit databus and        *\n\r"
          "*         4x1MByte NOR flash banks)          *\n\r"
  ...
  static const unsigned char message[] =
    "\n\n\r**********************************************\n\r"
          "* LPC24x8 OEM Board from Embedded Artists AB *\n\r"
          "*              EA-version: 1.9               *\n\r"
          "*     (with support for 16-bit databus)      *\n\r"
          "*          www.EmbeddedArtists.com           *\n\r"
          "**********************************************\n\r"
          "Booting (may take some time).";
#endif

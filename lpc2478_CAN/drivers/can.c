...
int can_open(void *ipbase, signed int arg)
{
   ...
   if (cancfg != (CAN_CFG_T *) NULL)
   {
      ...
      if (cancfg->init == FALSE)
      {
         ...
         /* Install CAN interrupt handlern note! this can be called multiple times */
...
int can_read(int devid, void *buffer, int max_bytes)
{
   ...
   if (cancfgptr->init == TRUE)
   {
      ...
      while ((max_bytes > 0)
            && (cancfgptr->rb.rx_tail != cancfgptr->rb.rx_head))
      {
...
#ifdef CAN_1_ENABLE
/**
 *  Interrupt Service Routine for RX interupt CAN 1
 */
void CANISR_Rx1(void)
{
   can_standard_receive(&can1cfg);
   /* Acklowledge interupt */
   tmpcanregs = can1cfg.regptr;
   CanInt = tmpcanregs->canicr.TI1;
}

/**
 *  Interrupt Service Routine for TX interupt CAN 1
 */
void CANISR_Tx1(void)
{
   can_standard_transmit(&can1cfg);
...
#ifdef CAN_2_ENABLE
/**
 *  Interrupt Service Routine for RX interupt CAN 2
 */
void CANISR_Rx2 ( void )
{
   can_standard_receive(&can2cfg);
   /* Acknowledge interupt */
   tmpcanregs = can2cfg.regptr;
...
#ifdef CAN_3_ENABLE
/**
 *  Interrupt Service Routine for RX interupt CAN 3
 */
void CANISR_Rx3 ( void )
{
   can_standard_receive(&can3cfg);
   /* Acknowledge interupt */
   tmpcanregs = can3cfg.regptr;
   CanInt = tmpcanregs->canicr.TI1;
}

/**
 *  Interrupt Service Routine for TX interupt CAN 3
 */
void CANISR_Tx3 ( void )
{
   can_standard_transmit(&can3cfg);
   /* Acknowledge interupt */
   tmpcanregs = can3cfg.regptr;
   CanInt = tmpcanregs->canicr.TI1;
}
...
#ifdef CAN_4_ENABLE
/**
 *  Interrupt Service Routine for RX interupt CAN 4
 */
void CANISR_Rx4 ( void )
{
   can_standard_receive(&can4cfg);
   /* Acknowledge interupt */
   tmpcanregs = can4cfg.regptr;
   CanInt = tmpcanregs->canicr.TI1;
}
 ...
 *  Interrupt Service Routine for TX interupt CAN 1
 */
void CANISR_Tx4 ( void )
{
   can_standard_transmit(&can4cfg);
   /* Acknowledge interupt */
   tmpcanregs = can4cfg.regptr;
   CanInt = tmpcanregs->canicr.TI1;
}

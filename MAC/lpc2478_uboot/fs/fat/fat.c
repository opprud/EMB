...
#else
		/* we cannot use the code above since we get an alignment */
		/* assume little endian */		
		ret = 
		  mydata->fatbuf[offset*4] +
		  ((mydata->fatbuf[offset*4 + 1]) << 8) +
		  ((mydata->fatbuf[offset*4 + 2]) << 16) +
		  ((mydata->fatbuf[offset*4 + 3]) << 24);
	...
	do {
		...
		while(actsize < filesize) {
			...
			if((newclust -1)!=endclust)
				goto getit;
			if (newclust <= 0x0001 || newclust >= mydata->clusterLimit) {
...
getit:
		...
		if (curclust <= 0x0001 || curclust >= mydata->clusterLimit) {
			FAT_DPRINT("curclust: 0x%x\n", curclust);
/* TO-DO EA Why??? */
...
__u8 do_fat_read_block[MAX_CLUSTSIZE] __attribute__ ((aligned(4)));  /* Block buffer */
long
	     ...
	     int dols)
{
    ...
    boot_sector bs;
    volume_info volinfo;
    fsdata datablock __attribute__ ((aligned(4)));
	...
	{
	case 32: 
		mydata->clusterLimit = 0x0ffffff0;
		break;
	case 16:
		mydata->clusterLimit = 0xfff0;
		break;
	case 12:
		mydata->clusterLimit = 0x0ff0;
		break;
	}
